...
void ParticleFilter::dataAssociation(Map map_landmarks, std::vector<LandmarkObs>& observations_transf, double sensor_range, Particle& particle) {
	...
	{
		double dist_lm_p = dist(particle.x, particle.y, map_landmarks.landmark_list[i].x_f, map_landmarks.landmark_list[i].y_f);
		...
		if (dist_lm_p < sensor_range)
		{
			...
			distances[0] = dist(observations_transf[0].x, observations_transf[0].y, map_landmarks.landmark_list[i].x_f, map_landmarks.landmark_list[i].y_f);
			min_dist_id = 0;
			for (int k = 1; k < observations_transf.size(); k++)
			{
				distances[k] = dist(observations_transf[k].x, observations_transf[k].y, map_landmarks.landmark_list[i].x_f, map_landmarks.landmark_list[i].y_f);
			...
			observations_transf[min_dist_id].id = i + 1;
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
	...
	for (size_t i = 0; i < num_particles; i++)
	{
		...
		// Associate Landmarks with closest transfered observation by nearest neighbor method
		...
		for (int a = 0; a < observations.size(); a++)
		{
			int associ_lm = observations_transf[a].id;
			if (associ_lm > 0)
			{
				double x = observations_transf[a].x;
				double y = observations_transf[a].y;
				double x_pred = map_landmarks.landmark_list[associ_lm - 1].x_f;
				double y_pred = map_landmarks.landmark_list[associ_lm - 1].y_f;

				// calculate new weights using the multivariante Gaussian probability density function
				GaussProb *= exp(-0.5 * ((pow((x - x_pred), 2) / (std_landmark[0]* std_landmark[0])) + (pow((y - y_pred), 2) / (std_landmark[1]* std_landmark[1])))) / (2 * M_PI*std_landmark[0] * std_landmark[1]);
			}

		}
		//saving the probability as weights
		particles[i].weight = GaussProb;
		weights[i] = GaussProb;
	}
...
void ParticleFilter::resample() {
	...
	// Spinning the Resampling Wheel
	// Resample the particles based on their weights, particles with less weight will disapear faster
